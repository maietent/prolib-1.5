...
void ZGame::StartRecording()
{
	...
	else if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_INFECTION)
	{
		...
		nWritten = zfwrite(&pRule->GetSpies(), sizeof(MUID), (int)Count, m_pReplayFile);
		if (nWritten == 0)
			goto RECORDING_FAIL;
		...
		nWritten = zfwrite(&pRule->GetSpyItems(), sizeof(MTD_SpyItems), (int)spyItemCount, m_pReplayFile);
		if (nWritten == 0)
			goto RECORDING_FAIL;
	}
	...
	if (ZGetGameClient()->GetMatchStageSetting()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
	{
		...
		int npcCount = (int)ZGetObjectManager()->GetNPCObjectMap()->size();
