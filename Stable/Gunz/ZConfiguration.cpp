...
bool ZConfiguration::LoadConfig(const char* szFileName)
{
	...
	for (auto itor = rootNode->first_node(); itor; itor = itor->next_sibling())
	{
		...
		if (_stricmp(itor->name(), ZTOK_CUSTOM) == 0)
		{
			...
			m_Video.bDxvk = atob(itor->first_node(ZTOK_ETC_DXVK)->value());
			m_Etc.dynamicModels = atob(itor->first_node("DYNAMICMODELS")->value());
			m_Video.nFOV = atoi(itor->first_node(ZTOK_ETC_FOV)->value());
			m_Video.UnlockedDir = atoi(itor->first_node(ZTOK_ETC_UNLOCKEDDIR)->value());
		}
	}
...
bool ZConfiguration::SaveToFile(const char *szFileName, const char* szHeader)
{
	...
	///CUSTOM
	{
		...
		childNode = doc.allocate_node(rapidxml::node_element, ZTOK_ETC_DXVK, m_Video.bDxvk ? "true" : "false");
		parentNode->insert_node(0, childNode);
		...
		childNode = doc.allocate_node(rapidxml::node_element, ZTOK_ETC_UNLOCKEDDIR, doc.allocate_string(to_string(m_Video.UnlockedDir).c_str()));
		parentNode->insert_node(0, childNode);
	}
...
void ZConfiguration::Init()
{
	...
	m_Etc.bTrailOption = false; //Trail Option
