...
void ZApplication::ParseStandAloneArguments(const char* pszArgs)
{
	...
	if ( str != NULL) {
		...
		if(GetNextName(buffer,sizeof(buffer),(char*)str+strlen(ZTOKEN_GAME)))
		{
	...
	if (str != nullptr) {
		...
		if (first != std::string::npos) {
			...
			if (last != std::string::npos) {
				mapName = input.substr(first + 1, last - (first + 1));
			}
		}

		mlog("how about we print mapname! %s \n", mapName.c_str());
		...
		if (sscanf(str, "%255s %255s %d", szTemp, szMap, &nDummyCount) == 3) {
			SetLaunchMode(ZLAUNCH_MODE_STANDALONE_GAME);
			CreateTestGame(const_cast<char*>(mapName.c_str()), nDummyCount, true);
		}
		else {
			mlog("Failed to parse test game:(\n");
		}
		return;
	}
