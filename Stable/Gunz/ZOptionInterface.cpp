...
void ZOptionInterface::InitInterfaceOption(void)
{
	...
	*/
	...
	//	ComboBox
	{
		...
		pWidget = (MComboBox*)pResource->FindWidget("RenderingApi");
		if (pWidget)
		{
			if (ZGetConfiguration()->GetVideo()->bDxvk)
			{
	...
	//	Button
	{
		...
		pWidget = (MButton*)pResource->FindWidget("UnlockedDir");
		if (pWidget) {
			pWidget->SetCheck(Z_ETC_UNLOCKEDDIR);
		}
...
//		ZGetInput()->UnregisterActionKey(i);
	...
	*/
	...
	}
	{
		...
		pWidget = (MButton*)pResource->FindWidget("UnlockedDir");
		if (pWidget) {
			Z_ETC_UNLOCKEDDIR = pWidget->GetCheck();
		}
	...
	{	// Etc
		...
		pComboBox = (MComboBox*)pResource->FindWidget("RenderingApi");
		if (pComboBox)
		{
			bool isEnabledDxvk = pComboBox->GetSelIndex();
			if (Z_VIDEO_DXVK != isEnabledDxvk) {
				ZApplication::GetGameInterface()->ShowMessage("Please restart the client for the changes to take effect.");
				Z_VIDEO_DXVK = pComboBox->GetSelIndex();
			}
		}
